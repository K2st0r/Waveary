...
- continue hardening early-acquaintance inference beyond emotional-state false positives, identity-style self-description, quoted name-sharing, and `my name's` introductions into the next bounded ambiguous introduction cases, but keep the parser narrow enough that ordinary emotional sentences are never treated as confirmed names
