...
- `waveary-core`
  ...
  - the next reply-realism pass now also treats light status-update turns such as `I'm home now`, `I'm back`, and `just finished` as their own ordinary-chat subtype, so both live-provider prompts and scripted fallback replies answer them with a short warm acknowledgment instead of stretching them into assistant-like recap or follow-up cadence
  - early-acquaintance name inference now also rejects light status-update tokens such as `home` and `done`, so ordinary check-in texts no longer pollute remembered preferred-name state
...
- translate the new market benchmark into one next concrete product-system step, prioritizing a user-facing layered-memory or summary-control surface now that concept-level identity memory exists internally
...
- continue the dialogue-quality pass by extending the new status-update-aware short-texting cadence beyond `home / back / done` into the next bounded low-stakes everyday-message buckets, while keeping early-name inference narrow enough that casual status words never become remembered identity
