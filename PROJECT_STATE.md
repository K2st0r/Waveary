...
- `waveary-core`
  ...
  - the repository now also records a dedicated open-source benchmark note so future shell acceleration can distinguish permissive direct-reuse sources from GPL or custom-license reference-only sources instead of re-evaluating that legal boundary from scratch
...
- use `docs/open-source-benchmark.md` as the starting filter before any future third-party code borrowing, and prefer permissive-license sources such as `LibreChat` before reference-only sources such as `Chatbox CE` or custom-license projects
