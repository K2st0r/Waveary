...
1. Start the first `waveary-voice` delivery path.
   Status: in progress
   Current state: a dedicated `waveary-voice` workspace now exists, the first browser-native TTS planner is implemented, `waveary-web` exposes `/api/voice/speak`, the chat page can already auto-speak or manually speak the latest reply, that route now prefers a real OpenAI-compatible `/audio/speech` TTS call before falling back to browser speech planning, provider-backed TTS now has explicit saved voice config plus quality-oriented voice presets exposed through `/api/voice/config` and the compact chat voice strip, the chat page has a browser-native microphone input path that drafts live speech into the composer and auto-sends the final recognized turn through the existing chat flow, that path now also supports a persistent live voice conversation loop that auto-resumes listening after each spoken reply, the console init path now tolerates a missing `/api/voice/config` route so voice-route startup drift no longer breaks provider setup, ???? can now be configured through a dedicated voice provider instead of being forced to reuse the chat provider, the first dedicated domestic adapter now exists through Doubao TTS with `appId / cluster / voice` fields, and the first self-hosted cut now exists through a generic local HTTP bridge with `engine / endpointPath / speaker / referenceVoiceId` fields.
   Next cut: run a focused browser pass on both dedicated Doubao and dedicated local self-hosted voice paths, then choose deliberately between provider-backed STT and a truer realtime duplex / interruption pass.
