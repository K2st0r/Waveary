...
3. Keep polishing the `waveary-web` home / console shell.
   Status: in progress
   Current state: the homepage first screen is tighter, the hero fits better on common desktop heights, the doodle layer uses a broader black-and-white asset set, the README hero banner has been rebuilt into a centered transparent portrait-fan asset for cleaner GitHub rendering, the console uses a denser control-desk shell, the provider/model setup remains visible through dedicated status surfacing, the chat composer keeps settings on the left and actions on the right, the provider workspace now clearly distinguishes unsaved draft credentials from the active saved runtime config, the old top runtime navigation has now been replaced by one OpenClaw-style left app sidebar shared by chat and console, the in-product default flow now opens on `#chat`, the session layer now persists companion-profile setup end to end, the server now preserves companion-profile archives through create/update/export/import flows, the server now has a saved non-chat model capability config foundation for speech / vision / image / video, the chat shell keeps the intrusive persona header out of the main canvas, the sidebar now uses title-only direct labels without duplicate `进入对话 / Open chat` routing, the shorter desktop-height and narrow client-window sidebar hit areas now verify cleanly in browser checks, the chat utility rail remains visible as a stable narrow right-side strip, and non-provider console workspaces are no longer forced back to provider setup while chat readiness is settling.
   Latest cut: the client structure now keeps companion profile, model capability setup, and skills as separate workspaces rather than mixing them into one settings wall; the chat empty state now links lightly into profile setup instead of duplicating the full editor.
   Next cut: visually inspect the empty-chat profile shortcut and sessions/profile proportions in the fresh desktop dev window, then continue wiring server-side companion-profile archive and model capability config details without reintroducing long mixed control walls.

4. Make GitHub onboarding usable for non-developers.
   Status: in progress
   Current state: the GitHub-facing onboarding path now includes separate English and Simplified Chinese README / deployment pages, explicit code-vs-brand boundary docs through `TRADEMARKS.md`, `BRAND-ASSETS.md`, and `NOTICE`, a dedicated commercialization page, a dedicated editions page, and a corrected Chinese README license section that no longer falsely says `MIT`.
   Next cut: visually review the rendered GitHub pages as a whole and decide whether the next repo-facing doc pass should add screenshots, a contact channel, or a more polished top-of-readme project overview block.
