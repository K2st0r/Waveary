...
3. Keep polishing the `waveary-web` home / console shell.
   Status: in progress
   Current state: the homepage first screen is tighter, the hero fits better on common desktop heights, the doodle layer uses a broader black-and-white asset set, the README hero banner has been rebuilt into a centered transparent portrait-fan asset for cleaner GitHub rendering, the console uses a denser control-desk shell, the provider/model setup remains visible through dedicated status surfacing, the chat composer keeps settings on the left and actions on the right, the provider workspace now clearly distinguishes unsaved draft credentials from the active saved runtime config, the old top runtime navigation has now been replaced by one OpenClaw-style left app sidebar shared by chat and console, the in-product default flow now opens on `#chat`, the session layer now persists companion-profile setup end to end, the chat shell keeps the intrusive persona header out of the main canvas, the sidebar now uses title-only direct labels without duplicate `进入对话 / Open chat` routing, the shorter desktop-height and narrow client-window sidebar hit areas now verify cleanly in browser checks, and the chat utility rail remains visible as a stable narrow right-side strip.
   Next cut: visually inspect the installed desktop-window proportions after the sidebar breakpoint fix, then do one tighter density and proportion pass on `sessions/profile` and the reshaped provider workspace, especially the still-wide empty center area in the sessions stage, any remaining lower-stage whitespace that survives the grouped-shell balance pass, and any session-list visual noise that still competes with the active control stage.
