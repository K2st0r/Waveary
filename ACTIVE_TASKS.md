...
3. Keep polishing the `waveary-web` home / console shell.
   Status: in progress
   Current state: the homepage first screen is tighter, the hero fits better on common desktop heights, the doodle layer uses a broader black-and-white asset set, the README hero banner has been rebuilt into a centered transparent portrait-fan asset for cleaner GitHub rendering, the console uses a denser control-desk shell, the provider/model setup remains visible through dedicated status surfacing, the chat composer keeps settings on the left and actions on the right, the provider workspace now clearly distinguishes unsaved draft credentials from the active saved runtime config, the old top runtime navigation has now been replaced by one OpenClaw-style left app sidebar shared by chat and console, the in-product default flow now opens on `#chat`, the session layer now persists companion-profile setup end to end, the chat shell keeps the intrusive persona header out of the main canvas, the sidebar now uses title-only direct labels without duplicate `进入对话 / Open chat` routing, and the non-provider control workspaces now follow one grouped client hierarchy with `Conversation / Sessions / Control / Settings` plus matching single-stage internal tabs for `skills / channels / settings`, with the sparsest grouped workspaces now rebalanced into fuller two-column stages after real browser verification.
   Next cut: do one tighter density and proportion pass on `sessions/profile` and the reshaped provider workspace, especially the still-wide empty center area in the sessions stage, any remaining lower-stage whitespace that survives the new grouped-shell balance pass, and any session-list visual noise that still competes with the active control stage.
