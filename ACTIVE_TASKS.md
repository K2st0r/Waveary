...
1. Start the first `waveary-voice` delivery path.
   Status: in progress
   Current state: a dedicated `waveary-voice` workspace now exists, the first browser-native TTS planner is implemented, `waveary-web` exposes `/api/voice/speak`, the chat page now stays focused on live voice instead of redundant manual speak controls, provider-backed TTS has explicit saved voice config plus quality-oriented presets through `/api/voice/config`, dedicated voice mode can diverge from the chat provider, dedicated domestic voice already works through Doubao, the first self-hosted cut exists through a generic local HTTP bridge, normal chat turns now also emit a structured companion delivery hint that the browser planner plus provider-backed TTS paths consume directly for style, pace, closeness, and expressiveness, and the local self-hosted path now exposes richer engine-tuning fields in the existing chat voice strip instead of leaving them backend-only.
   Next cut: run a focused browser pass across shared, dedicated OpenAI-compatible, Doubao, and local self-hosted voice playback, with special attention to the new local tuning controls (`textLanguage / promptLanguage / referenceTranscript / stylePrompt / styleStrength / temperature / topP`), before choosing between provider-backed STT and a truer realtime duplex / interruption pass.
