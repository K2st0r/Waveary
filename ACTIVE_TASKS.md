...
1. Start the first `waveary-voice` delivery path.
   ...
   Next cut: run a real-browser verification pass for Gemini dedicated voice with both the official preset and the Micu relay preset, then decide whether the next provider-specific voice cut should be Fish network recovery, Micu/Gemini polish, Doubao/local STT support, or interruption/full duplex work.
