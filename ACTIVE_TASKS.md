...
3. Keep polishing the `waveary-web` home / console shell.
   ...
   Next cut: wire the new companion-profile archive and model capability config server foundations into the visible `sessions/profile` and provider/model control UI, then verify the same console workspace behavior through `npm run desktop:dev`.
