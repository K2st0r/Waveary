...
3. Keep polishing the `waveary-web` home / console shell.
   Status: in progress
   Current state: the homepage first screen is tighter, the hero fits better on common desktop heights, the doodle layer uses a broader black-and-white asset set, the README hero banner has been rebuilt into a centered transparent portrait-fan asset for cleaner GitHub rendering, the console uses a denser control-desk shell, the provider/model setup remains visible through dedicated status surfacing, the chat composer keeps settings on the left and actions on the right, the provider workspace now clearly distinguishes unsaved draft credentials from the active saved runtime config, the old top runtime navigation has now been replaced by one OpenClaw-style left app sidebar shared by chat and console, the in-product default flow now opens on `#chat`, the session layer now persists companion-profile setup end to end, the chat shell keeps the intrusive persona header out of the main canvas, the sidebar now uses title-only direct labels without duplicate `进入对话 / Open chat` routing, the shorter desktop-height and narrow client-window sidebar hit areas now verify cleanly in browser checks, the chat utility rail remains visible as a stable narrow right-side strip, and non-provider console workspaces are no longer forced back to provider setup while chat readiness is settling.
   Next cut: verify the same console workspace behavior through `npm run desktop:dev`, then visually inspect the installed desktop-window proportions after the sidebar breakpoint fix and do one tighter density pass on `sessions/profile` plus the reshaped provider workspace.
